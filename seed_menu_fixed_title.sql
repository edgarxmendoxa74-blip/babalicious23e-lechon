...
ALTER TABLE menu_items ADD COLUMN IF NOT EXISTS title TEXT; -- Ensure title exists (it does, but safe to ask)
ALTER TABLE menu_items ADD COLUMN IF NOT EXISTS description TEXT;
ALTER TABLE menu_items ADD COLUMN IF NOT EXISTS price NUMERIC;
...
ALTER TABLE menu_items ADD COLUMN IF NOT EXISTS variations JSONB DEFAULT '[]'::jsonb;
ALTER TABLE menu_items ADD COLUMN IF NOT EXISTS addons JSONB DEFAULT '[]'::jsonb;
...
-- Also try deleting by title or name to be clean if 'category' column wasn't populated before
DELETE FROM menu_items WHERE title IN ('Whole Lechon', 'Lechon Belly', 'Party Box');

-- 4. INSERT NEW ITEMS
-- We insert into BOTH 'name' and 'title' to be safe since 'title' is NOT NULL.

-- Whole Lechon
INSERT INTO menu_items (category_id, name, title, description, price, image, sort_order, variations)
VALUES (
...
-- Party Box
INSERT INTO menu_items (category_id, name, title, description, price, image, sort_order, variations, addons)
VALUES (
