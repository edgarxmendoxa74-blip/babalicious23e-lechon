...
-- 2. Clear old items (using the simplified 'category' column if applicable, or just matching on category ID lookup)
-- IMPORTANT: We need to know if 'category_id' exists. If not, and it's 'category', we use that.
-- Based on error, we assume column is 'category' which might store the ID or Name. 

-- Let's try to be safe: DELETE based on category lookup
DELETE FROM menu_items WHERE category_id IN (SELECT id FROM categories WHERE name IN ('Whole Lechon', 'Lechon Belly', 'Party Box'));

-- 3. Insert New Items
-- We are assuming column is 'category' (UUID/Text) based on the specific error 'Perhaps you meant to reference the column "menu_items.category"'.

-- Whole Lechon
INSERT INTO menu_items (category_id, name, description, price, image, sort_order, variations)
VALUES (
    (SELECT id FROM categories WHERE name = 'Whole Lechon'),
    'Whole Lechon',
...
INSERT INTO menu_items (category_id, name, description, price, image, sort_order, variations)
VALUES (
    (SELECT id FROM categories WHERE name = 'Lechon Belly'),
    'Lechon Belly',
...
VALUES (
    (SELECT id FROM categories WHERE name = 'Party Box'),
    'Party Box',
